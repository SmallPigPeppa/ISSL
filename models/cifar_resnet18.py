...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        '''
        self.conv1_bn1 = Conv3x3_BN(inplanes,planes,stride)
        self.relu = nn.ReLU(inplace=True)
        self.conv2_bn2=Conv3x3_BN(planes,planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1_bn1 = Conv1x1_BN(inplanes, width)
        self.conv2_bn2 = Conv3x3_BN(width, width, stride, groups, dilation)
        self.conv3_bn3 = Conv1x1_BN(width, planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1_bn1=Conv3x3_BN(in_planes=3,out_planes=64,stride=1,dilation=2)
        self.relu = nn.ReLU(inplace=True)
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck):
                    nn.init.constant_(m.conv3_bn3.bn.weight, 0)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            '''
            downsample=Conv1x1_BN(self.inplanes, planes * block.expansion, stride)
